...
def sequence_to_true_signal(input_part=('ATGC', 1), output_folder='signal', sigroot='signal'):
    '''
    input_part, kmer_poremodel='null', perfect=0, p_len=1,
    ...
    '''
    #--- make integer -------#
    indep_result, final_ali, event_idx = repeat_n_time(0.1, mean_result, 1, seed=0)
    np.random.seed(0)
    event_std = np.random.uniform(-1 * event_std * std_result[event_idx], event_std * std_result[event_idx])
    final_result = mean_result[event_idx] + event_std
